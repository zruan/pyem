...
def augment_star_ucsf(df, inplace=True):
    ...
    if Relion.IMAGE_NAME in df:
        df[[UCSF.IMAGE_INDEX, UCSF.IMAGE_PATH]] = df[Relion.IMAGE_NAME].str.split("@", expand=True)
    ...
    if Relion.IMAGE_ORIGINAL_NAME in df:
        df[[UCSF.IMAGE_ORIGINAL_INDEX, UCSF.IMAGE_ORIGINAL_PATH]] = df[Relion.IMAGE_ORIGINAL_NAME].str.split("@", expand=True)
